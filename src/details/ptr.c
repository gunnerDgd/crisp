...
bool_t
	__ptr_seek
		(__ptr* par, i64_t par_cur) {
			if ((par->cur + par_cur) > par->mem->size) return false_t;
...
bool_t
	__ptr_rd8
		(__ptr* par, u8_t* par_buf) {
			...
			if ((par->cur + sizeof(u8_t)) > par->mem->size) return false_t;
...
bool_t
	__ptr_rd16
		(__ptr* par, u16_t* par_buf) {
			if (!par->mem)									 return false_t;
			if ((par->cur + sizeof(u16_t)) > par->mem->size) return false_t;
...
bool_t
	__ptr_rd32
		(__ptr* par, u32_t* par_buf) {
			if (!par->mem)									 return false_t;
			if ((par->cur + sizeof(u32_t)) > par->mem->size) return false_t;
...
bool_t
	__ptr_rd64
		(__ptr* par, u64_t* par_buf) {
			if (!par->mem)									 return false_t;
			if ((par->cur + sizeof(u64_t)) > par->mem->size) return false_t;
...
bool_t
	__ptr_copy
		(__ptr* par, __ptr* par_copy, u64_t par_len) {
			...
			if ((par->cur + par_len) > par->mem->size)		     return false_t;
			if ((par_copy->cur + par_len) > par_copy->mem->size) return false_t;
...
bool_t
	__ptr_gt
		(__ptr* par, __ptr* par_cmp, u64_t par_len) {
			...
			if((par	   ->cur + par_len) > par	 ->mem->size) return false_t;
			if((par_cmp->cur + par_len) > par_cmp->mem->size) return false_t;
