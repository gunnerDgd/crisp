...
#include "preset/lib/cstd/alloc.h"
#include "preset/lib/cstd/mem.h"
...
bool_t
    alloc_new
        (alloc* par, alloc_trait par_trait)               {
            ...
            return (par_trait) ? __alloc_new(par, par_trait) : false_t;
}

bool_t
    alloc_clone
        (alloc * par, alloc *par_clone) {
            return __alloc_clone(par, par_clone);
}
...
u64_t alloc_unit_size (alloc* par) { return ((__alloc*)par)->trait->unit_size ; }
u64_t alloc_unit_align(alloc* par) { return ((__alloc*)par)->trait->unit_align; }
u64_t alloc_max_size  (alloc* par) { return ((__alloc*)par)->trait->max_size  ; }

alloc*      get_alloc      () { return global_alloc      ; }
alloc_trait get_alloc_trait() { return global_alloc_trait; }
