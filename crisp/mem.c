...
c_u64_t
    c_mem_copy
        (c_mem_t par_dst, c_u64_t par_dst_off, c_mem_t par_src, c_u64_t par_src_off) {
            return mem_copy(par_dst.ptr, par_dst_off, par_src.ptr, par_src_off);
}

c_u64_t
    c_mem_copy_from
        (c_mem_t par_dst, c_u64_t par_dst_off, void* par_src, c_u64_t par_src_size) {
            return mem_copy_from(par_dst.ptr, par_dst_off, par_src, par_src_size);
}

c_u64_t
    c_mem_copy_to
        (c_mem_t par_src, c_u64_t par_src_off, void* par_dst, c_u64_t par_dst_size) {
            return mem_copy_to(par_src.ptr, par_src_off, par_dst, par_dst_size);
}
