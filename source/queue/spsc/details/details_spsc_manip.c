        ...
        (__atomic_spsc* pSpsc)
{
    ...
    InterlockedExchange64
        (&pSpsc->ptr_spsc_circular->ptr_cqueue_rdptr,
            pSpsc->ptr_spsc_circular->ptr_cqueue_rdptr->ptr_cqueue_next);
#endif
    ptr_spsc_rdptr_data
        = pSpsc->ptr_spsc_circular
                ->ptr_cqueue_rdptr->ptr_cqueue_data;
...
bool
    __atomic_spsc_write_to
        (__atomic_spsc* pSpsc,
         void*          pSpscWriteData)
{
    if(pSpsc->ptr_spsc_circular
        ->ptr_cqueue_wrptr->ptr_cqueue_next
            == pSpsc->ptr_spsc_circular->ptr_cqueue_rdptr)
                return false;
    
    pSpsc->ptr_spsc_circular
        ->ptr_cqueue_wrptr->ptr_cqueue_data
            = pSpscWriteData;
...
#elif  ATOMIC_STRUCTURE_BUILD_ENVIRONMENT_WINDOWS
    InterlockedExchange64
        (&pSpsc->ptr_spsc_circular->ptr_cqueue_wrptr,
            pSpsc->ptr_spsc_circular->ptr_cqueue_wrptr->ptr_cqueue_next);
#endif
    return true;
}
